...
class JumpNode(Node):
    def __init__(self):
        ...
        # Jump state machine
        self.jump_phase = 'idle'  # 'idle', 'crouch', 'explode', 'retract', 'hold'
        self.t = 0.0
        ...
        self.explode_time = 0.1 # Fast explosion - 50ms
        self.retract_time = 0.05  # Fast retraction - 50ms
    ...
    def cmd_callback(self, msg):
        ...
        if cmd == 'jump':
            if not self.jump_active:
                self.get_logger().info('JUMP command received! Starting jump sequence...')
    ...
    def timer_callback(self):
        ...
        # Only publish if current command is 'jump'
        if self.current_cmd != 'jump':
            return
        
        # Execute jump state machine
        if self.jump_phase == 'idle':
            ...
            if alpha >= 1.0:
                # Extension complete, retract!
                self.jump_phase = 'retract'
                self.t = 0.0
                target_position = self.extend_position
                self.get_logger().info('Phase: RETRACT - Pulling legs back!')
            else:
                # Linear interpolation for explosive motion
            ...
            # FAST retraction back to crouch
            alpha = self.t / self.retract_time
            
            if alpha >= 1.0:
                # Retraction complete, hold!
                self.jump_phase = 'hold'
                self.t = 0.0
                target_position = self.crouch_position
                self.get_logger().info('Phase: HOLD - Landing and holding crouch position')
            else:
                # Linear interpolation for fast retraction
                target_position = self.interpolate_linear(
                    self.extend_position,
                    self.crouch_position,
                    alpha
            ...
            if self.t >= self.hold_time:
                # Jump sequence complete - go to idle
                self.jump_phase = 'idle'
                self.jump_active = False
                self.current_cmd = 'idle'
                self.t = 0.0
                self.get_logger().info('Jump sequence COMPLETE! Jump node going idle.')
